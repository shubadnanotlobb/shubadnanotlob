...
let allRestaurants = [];
let allCategories = [];
let navigationHistory = ['pageHome']; // سجل التصفح لدعم زر الرجوع الذكي
...
// 2. Navigation Control (إظهار وإخفاء الصفحات والتحكم بالسهم)
function showPage(pageId, pushToHistory = true) {
    ...
    if (pushToHistory && navigationHistory[navigationHistory.length - 1] !== pageId) {
        navigationHistory.push(pageId);
    }

    // التحكم بظهور زر الرجوع (السهم)

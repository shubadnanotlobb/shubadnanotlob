...
async function saveCategoryToFirebase() {
    ...
    try {
        ...
        if (status) {
            status.innerText = "جاري رفع البيانات والتغيرات...";
            status.style.display = "block";
        }

        let finalImgUrl = urlInput || "";

        // إذا تم اختيار ملف من اللابتوب
...
function resetAdminForm() {
    ...
    const adminLogoFile = document.getElementById('adminLogoFileInput');
    const adminGallery = document.getElementById('adminGallery');
    const adminGalleryFiles = document.getElementById('adminGalleryFilesInput');
...
async function saveRestaurantToFirebase() {
    ...
    try {
        ...
        if (docId) {
            ...
            restaurantData.createdAt = serverTimestamp(); // ضمان عدم اختفاء المطعم
...
function editRestaurant(r) {
    ...
    document.getElementById('formTitle').innerText = 'Edit Restaurant';
}
...
function renderRestaurantsList(searchQuery = '') {
    ...
    if (filtered.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: #777; width: 100%;">لا توجد مطاعم حالياً في هذا القسم.</p>';
        return;
    }

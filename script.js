...
async function saveCategoryToFirebase() {
  ...
  try {
    ...
    // إذا كان قسماً جديداً، نقوم بإنشاء مرجع مع معرف أولاً تماماً مثل المطاعم
    if (!categoryId) {
...
function editCategory(categoryObjOrId, nameAr, nameEn, imgUrl) {
  let id, cNameAr, cNameEn, cImgUrl, cImgPath;
  
  if (typeof categoryObjOrId === 'object' && categoryObjOrId !== null) {
    id = categoryObjOrId.id;
    cNameAr = categoryObjOrId.nameAr;
    cNameEn = categoryObjOrId.nameEn;
    cImgUrl = categoryObjOrId.imgUrl;
    cImgPath = categoryObjOrId.imgStoragePath;
  } else {
    id = categoryObjOrId;
    cNameAr = nameAr;
    cNameEn = nameEn;
  ...
  if (adminCatAr) adminCatAr.value = cNameAr || "";
  if (adminCatEn) adminCatEn.value = cNameEn || "";
  if (adminCatImg) adminCatImg.value = cImgUrl || "";
  if (adminCatFile) adminCatFile.value = "";

  if (catPreview) {
    catPreview.src = cImgUrl || "";
    catPreview.style.display = cImgUrl ? "block" : "none";
  }
    ...
    onSnapshot(q, snapshot => {
      ...
      snapshot.forEach(docSnap => {
        ...
        if (adminCatContainer) {
          ...
          const catObjectString = JSON.stringify(categoryItem).replace(/"/g, "&quot;");

          item.innerHTML = `
            <span>${data.nameAr || ""} (${data.nameEn || ""})</span>
            <div>
              <button type="button" style="padding: 4px 8px; background: #008080; color: white; border: none; border-radius: 4px; cursor: pointer;" onclick="window.editCategory(${catObjectString})">تعديل</button>

...
async function saveCategoryToFirebase() {
    ...
    try {
        if (docId) {
            ...
            await addDoc(collection(db, "categories"), { nameAr, nameEn, imgUrl, createdAt: new Date() });
...
async function saveRestaurantToFirebase() {
    ...
    const restaurantData = { category, name, desc, logo, gallery: galleryArray, phone, menu, map, updatedAt: new Date() };
        ...
        snapshot.forEach((docSnap) => {
            ...
            if (adminCatContainer) {
                ...
                item.innerHTML = `
                    ...
                    <div>
                        <button type="button" style="padding: 4px 8px; background: #008080; color: white; border: none; border-radius: 4px; cursor: pointer;" onclick="editCategory('${id}', '${data.nameAr}', '${data.nameEn}', '${data.imgUrl || ''}')">تعديل</button>
                        <button type="button" style="padding: 4px 8px; background: #e74c3c; color: white; border: none; border-radius: 4px; cursor: pointer;" onclick="deleteCategoryFromFirebase('${id}')">حذف</button>
                    </div>
                `;
        ...
        snapshot.forEach((docSnap) => {
            ...
            if (adminRestContainer) {
                ...
                item.innerHTML = `
                    <span>${data.name}</span>
                    <div>
                        <button type="button" style="padding: 4px 8px; background: #e74c3c; color: white; border: none; border-radius: 4px; cursor: pointer;" onclick="deleteRestaurantFromFirebase('${id}')">حذف</button>
                    </div>
                `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // دعم اللمس للفوتر على الهواتف
    const footerBtn = document.getElementById('mainAppFooter');
    if (footerBtn) {
        footerBtn.addEventListener('click', (e) => { e.preventDefault(); checkAdminAccess(); });
        footerBtn.addEventListener('touchend', (e) => { e.preventDefault(); checkAdminAccess(); });
    }
});

...
async function saveRestaurantToFirebase() {
  ...
  try {
    ...
    // NEW RESTAURANT
    // IMPORTANT:
    // Create the document reference first, but DO NOT create
    // the Firestore document yet.
    // ========================================================
    ...
    if (!docId) {
      ...
      // We use setDoc instead of creating and then
      // updateDoc-ing the same document.
    ...
    // If admin manually changed the gallery URLs,
    // use those values.
              ...
              item.innerHTML = `
                ...
                <div>
                  ...
                  <button
                    ...
                    onclick="window.editCategory('${id}', '${String(data.nameAr || "").replace(/'/g, "\\'")}', '${String(data.nameEn || "").replace(/'/g, "\\'")}', '${String(data.imgUrl || "").replace(/'/g, "\\'")}')"
                  >
                    تعديل
                  </button>
